from typing import TYPE_CHECKING, Any, List, Set, Tuple, Type, Union
...
try:
    ...
    try:
        ...
        try:
            ...
            raise ImportError(
                "Failed to import httpx and aiohttp, please install one of them to use this plugin."
            )
...
with namespace("tsugu") as tsugu_namespace:
    ...
    @(
        open_forward := _build(
            Command("开启车牌转发", "开启车牌转发", meta=meta),
            _config.tsugu_open_forward_aliases,
        )
    ).handle()
    ...
    @(
        close_forward := _build(
            Command("关闭车牌转发", "关闭车牌转发", meta=meta),
            _config.tsugu_close_forward_aliases,
        )
    ).handle()
    async def _(bot: Bot, event: Event) -> None:
        user_id = event.get_user_id()
        await close_forward.send(
            await switch_forward(_get_platform(bot), user_id, False)
        )

    @(
        bind_player := _build(
            Command("绑定玩家 [server_name:str]", "绑定玩家信息", meta=meta).usage(
    ...
    async def _(server_name: Match[str], bot: Bot, event: Event) -> None:
        if server_name.available:
            ...
            try:
                tsugu_user = await _get_tsugu_user(
                    _get_platform(bot), event.get_user_id()
                )
        ...
        return await player_bind(
            bind_player, _get_platform(bot), event.get_user_id(), _server
        )
    ...
    async def _(bot: Bot, event: Event, player_id: str = ArgPlainText()) -> None:
        ...
        try:
            response = await tsugu_api_async.bind_player_verification(
                _get_platform(bot), event.get_user_id(), server, int(player_id), "bind"
            )
        except FailedException as exception:
            return await bind_player.finish(exception.response["data"])

        await bind_player.send(
            f"绑定 {server_id_to_full_name(server)} 玩家 {player_id} 成功，正在生成玩家状态图片"
        )

        message = await search_player(
            _get_platform(bot), event.get_user_id(), int(player_id), server
        )
    ...
    async def _(server_name: Match[str], bot: Bot, event: Event) -> None:
        try:
            ...
            try:
                ...
                await bind_player.finish("错误: 服务器名未能匹配任何服务器")
        await main_server.finish(
            await switch_main_server(_get_platform(bot), event.get_user_id(), _server)
        )

    @(
        display_servers := _build(
            Command(
                "设置显示服务器 <server_list:str*>",
                "设定信息显示中的默认服务器排序",
                meta=meta,
            )
            .alias("默认服务器")
    ...
    async def _(server_list: list[str], bot: Bot, event: Event) -> None:
        ...
        await display_servers.finish(
            await set_default_servers(_get_platform(bot), event.get_user_id(), servers)
        )

    @(
        player_status := _build(
            Command(
                "玩家状态 [index:int] [server_name:str]",
                "查询自己的玩家状态",
                meta=meta,
            ).shortcut(
                r"(.+服)玩家状态$",
                {"args": ["{0}"], "command": f"{list(_command_start)[0]}玩家状态"},
            ),
    ...
    async def _(
        index: Match[int], server_name: Match[str], bot: Bot, event: Event
    ) -> None:
        if index.available:
            _index = index.result
        else:
        ...
        await player_status.finish(
            await player_info(
                _get_platform(bot), event.get_user_id(), _index, _server_name
            )
        )
    ...
    async def _(bot: Bot, event: Event) -> None:
        return await player_list.finish(
            await get_player_list(_get_platform(bot), event.get_user_id())
        )
    ...
    async def _(index: Match[int], bot: Bot, event: Event) -> None:
        return await switch_index.finish(
            await switch_player_index(
                _get_platform(bot), event.get_user_id(), index.result
            )
        )
    ...
    @(
        player_search := _build(
            Command(
                "查玩家 <player_id:int> [server_name:str]", "查询玩家信息", meta=meta
            )
            .alias("查询玩家")
    ...
    async def _(
        player_id: Match[int], server_name: Match[str], bot: Bot, event: Event
    ) -> None:
        ...
        await player_search.finish(
            await search_player(
                _get_platform(bot), event.get_user_id(), player_id.result, _server
            )
        )

    @(
        card_search := _build(
            Command("查卡 <word:str*>", "查卡", meta=meta)
            .alias("查卡牌")
    ...
    async def _(word: list[str], bot: Bot, event: Event) -> None:
        await card_search.finish(
            await search_card(_get_platform(bot), event.get_user_id(), " ".join(word))
        )
    ...
    @(
        character_search := _build(
            ...
            .example("查角色 吉他 :返回所有角色模糊搜索标签中包含吉他的角色列表"),
            _config.tsugu_search_character_aliases,
        )
    ).handle()
    async def _(word: list[str], bot: Bot, event: Event) -> None:
        await character_search.finish(
            await search_character(
                _get_platform(bot), event.get_user_id(), " ".join(word)
            )
        )
    ...
    async def _(word: list[str], bot: Bot, event: Event) -> None:
        await event_search.finish(
            await search_event(_get_platform(bot), event.get_user_id(), " ".join(word))
        )
    ...
    @(
        song_search := _build(
            ...
            .example("查曲 ag lv27 :返回所有难度为27的ag曲列表"),
            _config.tsugu_search_song_aliases,
        )
    ).handle()
    async def _(word: list[str], bot: Bot, event: Event) -> None:
        await song_search.finish(
            await search_song(_get_platform(bot), event.get_user_id(), " ".join(word))
        )
    ...
    @(
        chart_search := _build(
            ...
            .example("查谱面 1 :返回1号曲的所有谱面")
            .example("查谱面 1 expert :返回1号曲的expert难度谱面"),
            _config.tsugu_song_chart_aliases,
        )
    ).handle()
    async def _(
        song_id: Match[int], bot: Bot, event: Event, difficulty: str = "expert"
    ) -> None:
        try:
        ...
        await chart_search.finish(
            await song_chart(
                _get_platform(bot), event.get_user_id(), song_id.result, difficulty_id
            )
        )
    ...
    @(
        song_random := _build(
            ...
            .alias("随机")
            .example("随机曲 lv24 :在所有包含24等级难度的曲中, 随机返回其中一个")
            .example(
    ...
    async def _(word: list[str], bot: Bot, event: Event) -> None:
        await song_random.finish(
            await random_song(_get_platform(bot), event.get_user_id(), " ".join(word))
        )
    ...
    async def _(server_name: Match[str], bot: Bot, event: Event) -> None:
        ...
        await meta_search.finish(
            await song_meta(_get_platform(bot), event.get_user_id(), _server)
        )
    ...
    async def _(
        ...
        bot: Bot,
        event: Event,
        meta: Query[bool] = Query("meta.value", False),
    ) -> None:
        ...
        await stage_search.finish(
            await event_stage(
                _get_platform(bot), event.get_user_id(), _event_id, meta.result
            )
        )

    @(
        gacha_search := _build(
            Command("查卡池 <gacha_id:int>", "查卡池", meta=meta).usage(
                "根据卡池ID查询卡池信息"
            ),
    ...
    async def _(gacha_id: Match[int], bot: Bot, event: Event) -> None:
        await gacha_search.finish(
            await search_gacha(_get_platform(bot), event.get_user_id(), gacha_id.result)
        )

    @(
        ycx := _build(
            Command(
                "ycx <tier:int> [event_id:int] [server_name:str]",
                "查询指定档位的预测线",
                meta=meta,
            )
            .usage(
                "查询指定档位的预测线，如果没有服务器名的话，服务器为用户的默认服务器。"
    ...
    async def _(
        ...
        await ycx.finish(
            await search_ycx(
                _get_platform(bot), event.get_user_id(), tier.result, _event_id, _server
            )
        )
    ...
    @(
        ycx_all := _build(
            Command(
                "ycxall [event_id:int] [server_name:str]",
                "查询所有档位的预测线",
                meta=meta,
            )
            .usage(
                "查询所有档位的预测线，如果没有服务器名的话，服务器为用户的默认服务器。"
                "如果没有活动ID的话，活动为当前活动\n可用档线:\n{tier_list_of_server_to_string()}"
            )
            .alias("myycx"),
    ...
    async def _(
        event_id: Match[int], server_name: Match[str], bot: Bot, event: Event
    ) -> None:
        if event_id.available:
            _event_id = event_id.result
        else:
        ...
        await ycx_all.finish(
            await search_ycx_all(
                _get_platform(bot), event.get_user_id(), _server, _event_id
            )
        )
    ...
    @(
        lsycx := _build(
            Command(
                "lsycx <tier:int> [event_id:int] [server_name:str]",
                "查询指定档位的预测线",
                meta=meta,
            )
            .usage(
    ...
    async def _(
        ...
        await gacha_simulate.finish(
            await simulate_gacha(
                _get_platform(bot), event.get_user_id(), _times, _gacha_id
            )
        )
...
async def _(arp: Arparma) -> None:
    ...
    if (query := arp.all_matched_args["query"]) != "-1":
        for index, slot in enumerate(cmds):
            if query == slot.command or query in list(
                command_manager.get_shortcut(slot).keys()
            ):
                break
        else:
